...
class Game:
    def __init__(self):
        ...
        self.letters_text = UIElement(100, 10, "Not Enough Letters", WHITE)
                    ...
                    if len(self.text) == 5:
                        ...
                        if self.text == self.word or self.current_row + 1 == 6:
                            ...
                            if self.text != self.word:
                                self.end_screen_text = UIElement(100, 10, f"THE WORD WAS: {self.word}", WHITE)

                            # player win, send win message
                            else:
                                self.end_screen_text = UIElement(100, 10, "YOU GUESSED RIGHT", WHITE)
    ...
    def end_screen(self):
        play_again = UIElement(60, 460, "PRESS ENTER TO PLAY AGAIN", WHITE, 20)
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
